...
def getFeatureNames(annotation_data):
	...
	for i in range(0,attr_len):
		...
		gene_with_name = attr_info[1]	# or attr_info[2]; attr_info[0] is almost always the 'ID'
...
def savefile(annotation_data, fwd_annotation_data, rev_annotation_data, outfile):
	...
	if files_toSave == 'y' or files_toSave == 'Y':
		fwd_annotation_data.to_csv(outfile + 'forward_stranded' + '.csv', sep='\t', index=False)
		rev_annotation_data.to_csv(outfile + 'reverse_stranded' + '.csv', sep='\t', index=False)
		print("**** 2 files successfully extracted! ****")
...
def main():
	...
	parser.add_argument("-a", "--annotation", help="Input gene/transcript annotation file (in dataframe format) (.csv)")
	parser.add_argument("-f", "--features", help="Input countmatrix of genes/transcripts to be identified (.txt/.csv)")
	...
	# defining the inputs
	anno_df = args.annotation
	feats_df = args.features
	out_df = args.outfile

	modified_anno_df = getFeatureNames(anno_df)
	feats_list = feat_df.iloc[:,0]

	# extract out rows specific rows
	new_anno_df = modified_anno_df[modified_anno_df.feature_name.isin(feats_list)]
